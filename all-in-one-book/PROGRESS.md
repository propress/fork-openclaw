# 全书写作进度

## 状态说明
- ✅ 已完成并提交
- 🔄 进行中（当前轮次）
- ⏳ 待开始

## 进度表

| 章节 | 文件路径 | 状态 | 完成日期 |
|------|----------|------|----------|
| 第1章：系统全局架构 | 第一部分-架构全景/ch01-system-architecture.md | ✅ | 2026-03-27 |
| 第2章：入口与启动流程 | 第一部分-架构全景/ch02-entry-bootstrap.md | ✅ | 2026-03-27 |
| 第3章：类型系统与共享基础设施 | 第一部分-架构全景/ch03-types-infra.md | ✅ | 2026-03-27 |
| 第4章：Gateway 服务器 | 第二部分-Gateway控制平面/ch04-gateway.md | ✅ | 2026-03-27 |
| 第5章：消息路由引擎 | 第二部分-Gateway控制平面/ch05-routing.md | ✅ | 2026-03-27 |
| 第6章：Session 会话管理 | 第二部分-Gateway控制平面/ch06-sessions.md | ✅ | 2026-03-27 |
| 第7章：Pi Agent 核心 | 第三部分-Agent运行时/ch07-agent-runtime.md | ✅ | 2026-03-27 |
| 第8章：上下文引擎 | 第三部分-Agent运行时/ch08-context-engine.md | ✅ | 2026-03-27 |
| 第9章：工具系统 | 第三部分-Agent运行时/ch09-tools.md | ✅ | 2026-03-27 |
| 第10章：频道抽象层 | 第四部分-频道系统/ch10-channels.md | ✅ | 2026-03-27 |
| 第11章：典型频道实现 | 第四部分-频道系统/ch11-channel-impls.md | ✅ | 2026-03-27 |
| 第12章：媒体管道 | 第五部分-媒体与理解/ch12-media.md | ✅ | 2026-03-27 |
| 第13章：插件架构 | 第六部分-扩展与插件/ch13-plugins.md | ✅ | 2026-03-27 |
| 第14章：Skills 系统 | 第六部分-扩展与插件/ch14-skills.md | ✅ | 2026-03-27 |
| 第15章：安全模型 | 第七部分-安全系统/ch15-security.md | ✅ | 2026-03-27 |
| 第16章：macOS + Swabble | 第八部分-跨平台客户端/ch16-macos.md | ✅ | 2026-03-27 |
| 第17章：Android App | 第八部分-跨平台客户端/ch17-android.md | ✅ | 2026-03-27 |
| 第18章：Node 设备系统 | 第八部分-跨平台客户端/ch18-node-host.md | ✅ | 2026-03-27 |
| 第19章：Control UI 与 WebChat | 第九部分-前端/ch19-frontend.md | ✅ | 2026-03-27 |
| 第20章：Monorepo 与构建 | 第十部分-构建与工程/ch20-build.md | ✅ | 2026-03-27 |
| 第21章：关键路径追踪 | 第十一部分-端到端追踪/ch21-e2e-traces.md | ✅ | 2026-03-27 |
| 附录 | 附录/appendix.md | ✅ | 2026-03-27 |

## 下次续写指引

### 从哪里继续
- ✅ 全书 21 章 + 附录已全部完成

### 已知待补充项
- ch01 `src/auto-reply/reply/commands-bash.ts` 的具体 tool handler 注册方式 [需源码验证]
- ch02 `tryRouteCli()` 的具体内置别名列表 [需源码验证]
- ch03 `outbound/` 子模块中 delivery-queue 的重试策略细节 [需源码验证]
- ch04 Gateway 五阶段启动的具体行号和完整 sidecar 列表 [需源码验证]
- ch05 Identity Links 的具体实现细节（resolveLinkedPeerId 的缓存策略）[需源码验证]
- ch06 Queue Mode 的具体合并策略实现 [需源码验证]
- ch06 Session Patch 的增量 diff 算法 [需源码验证]
- ch07 具体的 Failover 切换逻辑（超时→切换 Provider 的判断标准）[需源码验证]
- ch08 Legacy 引擎具体的消息选择算法实现 [需源码验证]
- ch09 Canvas Host 的具体 A2UI 指令集 [需源码验证]
- ch09 Web Search Provider 完整列表 [需源码验证]
- ch10 ChannelHealthMonitor 的具体心跳协议 [需源码验证]
- ch11 Telegram Bot 命令的具体注册方式 [需源码验证]
- ch12 Provider Registry 的完整优先级规则 [需源码验证]
- ch14 Skill frontmatter 的具体字段列表 [需源码验证]
- ch14 skills/ 内置技能的完整列表 [需源码验证]
- ch16 macOS 应用的具体 SwiftUI 界面组件 [需源码验证]
- ch17 NodeRuntime.kt 的 Node.js 嵌入具体实现 [需源码验证]
- ch18 exec-policy 的具体策略规则 [需源码验证]
- ch19 事件类型完整列表 [需源码验证]

### 交接备忘（如需续写改进）
- 全书共 21 章 + 附录，总计 70+ Mermaid 流程图/序列图
- 较薄的章节（ch14 Skills、ch16 macOS、ch17 Android、ch18 Node Host）可以在后续轮次中加深
- 可能的改进方向：
  1. 补充 [需源码验证] 标注的项目
  2. 加深 ch14 Skills 系统（加入实际内置技能列表和注入流程）
  3. 加深 ch16-18 客户端章节（加入更多架构图和代码路径）
  4. 增加跨章节的交叉引用
  5. 全书总质检（覆盖率、一致性、流程图数量）

### 质量基线回顾（下次续写前必读）
- 全书术语约定：Gateway=网关控制平面, Agent=智能体运行时, Channel=频道适配层, Session=会话, Binding=路由绑定, Plugin=插件, Hook=生命周期钩子, Lane=命令车道, Context Engine=上下文引擎
- 流程图风格：Mermaid sequenceDiagram，参与者用模块名（如 "WhatsApp Extension", "Gateway", "Router", "Agent"）
- 调用路径风格：`src/xxx.ts::functionName() → src/yyy.ts::functionName()`
- 语言：中文正文，技术术语保留英文（首次出现附中文解释）
